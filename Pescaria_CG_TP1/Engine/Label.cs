...
namespace Pescaria_CG_TP1.Engine {
	public class Label : GameObject {
		public static string[] GetBrokenStrings (string text, int fontSize, float sizeX) {
			...
			int maxLength = (int) Math.Round(sizeX / fontSize * 1.7);
		...
		public Label (string text, int fontSize, Color color, Vector2 size, int padding = 0, int lineSpace = 10, string tag = "", Vector2 position = null, double rotation = 0, Vector2 scale = null) : base(size, tag, position, rotation, scale) {
			this.Text = text;
		...
		public override void OpenGLDraw (int glWidth, int glHeight) {
			...
			for (int i = 0; i < lines.Count; i++)
				gl.DrawText((int) Math.Max(0, this.Transform.Position.X + this.Transform.Size.X / 2f - (this.FontSize * (lines[i].Length / 1.7)) / 2f) + this.Padding, (int) Math.Max(0, (SceneManager.ScreenSize.Y - this.Transform.Position.Y - (this.Transform.Size.Y - this.FontSize * lines.Count - this.LineSpace * (lines.Count - 1)) / 2f) - (this.FontSize + this.LineSpace) * i), this.Color.R / 255f, this.Color.G / 255f, this.Color.B / 255f, "Arial", this.FontSize, lines[i]);
		}
	}
}
